...
```sh
$ git clone https://github.com/Nasferatuss/llama.cpp.git infcore-src
$ cd infcore-src
$ git checkout infcore
```

В закрытом контуре — с внутреннего зеркала/из архива, ветка та же.
...
Положите GGUF в `/opt/infcore/models` и снимите контрольные суммы — они пойдут в конфиг:

```sh
# chown infcore:infcore /opt/infcore/models/*.gguf
# chmod 0440 /opt/infcore/models/*.gguf
